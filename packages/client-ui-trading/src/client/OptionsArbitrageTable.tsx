/**
 * 套利机会表（2026-09-13 WB-13）：在 T 板内对当前期权链运行 scanArbitrage（平价 + 箱型，无风险）
 * 与 scanVerticalSpreads（方向性价差，非无风险），全部浏览器内计算（@dshtrading/strategies 纯库）。
 *
...
function kindLabel(kind: ArbitrageOpportunity['kind'], t: OptionsArbitrageTranslate): string {
  return kind === 'parity' ? t('options.arbitrage.kind.parity') : t('options.arbitrage.kind.box')
}
...
export function OptionsArbitrageTable({ t, chain, multiplier }: OptionsArbitrageTableProps): React.JSX.Element {
  const arb = useMemo(() => scanArbitrage(fromOptionChain(chain), { multiplier }), [chain, multiplier])
  ...
  return (
    <section className={css.root} data-dshtrading-options-arbitrage="">
      ...
      {arb.length === 0
        ...
        : (
          <table className={css.table}>
            ...
            <tbody>
              {arb.map((o, i) => (
                <tr key={i} data-kind={o.kind}>
                  <td>{kindLabel(o.kind, t)}</td>
      ...
      {arb.length > 0 && !allExecutable && (
        <div className={css.theory}>{t('options.arbitrage.theoryNote')}</div>
      )}

...
describe('cn_get_indicators 工厂', () => {
  it('默认全指标：6 个 id、22 条序列，MA5 latest 手算=397，取数参数透传', async () => {
    const { tool, getKlines } = makeTool()
    const text = await tool.execute({ symbol: '600519.SH', interval: '1d' }) as string
    ...
    expect(getKlines).toHaveBeenCalledWith('600519.SH', '1d', 300)
  })

  it('指定子集 + points 截尾：ma+rsi 共 7 条序列，tail 长度 = points', async () => {
    const { tool, getKlines } = makeTool()
    const text = await tool.execute({ symbol: '600519.SH', interval: '1h', indicators: 'ma,rsi', points: 5 }) as string
    ...
    expect(getKlines).toHaveBeenCalledWith('600519.SH', '1h', 300)
  })

  it('未知指标 id → 报错并列出可选集', async () => {
    const { tool } = makeTool()
    await expect(tool.execute({ symbol: '600519.SH', interval: '1d', indicators: 'ma,nope' }))
  ...
  it('空 K 线 → 报错；required 缺失由 dsh-tools 框架校验拦截', async () => {
    ...
    await expect(tool.execute({ symbol: '600519.SH', interval: '1d' })).rejects.toThrowError(/no klines/)
    await expect(tool.execute({ interval: '1d' })).rejects.toThrowError(/invalid arguments/)
    await expect(tool.execute({ symbol: '600519.SH' })).rejects.toThrowError(/invalid arguments/)
  })

  it('market 前缀默认 cn：cn_get_indicators', () => {
    const { tool } = makeTool()
    expect(tool.name).toBe('cn_get_indicators')
  })
})

...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/v1/instruments":
                self._send(200, encode(True, SERVICE.instruments(query.get("market", ""))))
                return
        except QuoteGatewayError as err:
            status = 400 if err.code == "BAD_REQUEST" else 200
            self._send(status, encode(False, {"code": err.code, "message": err.message}))
            return
        self._send(404, encode(False, {"code": "BAD_REQUEST", "message": f"unknown path {path}"}))
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            self._send(400, encode(False, {"code": "BAD_REQUEST", "message": f"invalid JSON: {err}"}))
            return
        if not isinstance(body, dict):
            self._send(400, encode(False, {"code": "BAD_REQUEST", "message": "request must be an object"}))
            return
        try:

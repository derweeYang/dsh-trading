...
def _post_then_abort(port: int, path: str) -> None:
    """发一个合法 POST 后立刻 RST 断开，模拟 urllib 30s 超时后的硬断开。"""
...
def test_client_gone_logs_single_line(serve, capsys):
    port = serve(_StubService(delay=0.3))
    _post_then_abort(port, "/v1/snapshot")

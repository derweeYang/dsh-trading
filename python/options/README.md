...
- `iquant` —— 经子进程调用 `dsh-iquant-quote`。注册表目前是 `510050`（沪）与 `159915`（深），`quotesSource=iquant_board`。本切面永远向 iquant-quote 发送 `source: synth`，不登录券商；SDK 接触只在 iquant-quote。深交所在该源上不再是 `szse_static_only`。`UNSUPPORTED` 收成 `NO_DATA` 并写明来自 iquant-quote。`asOf` 只属于 synth；定价 / IV 要求显式 `rate`。

合约存在两套代码体系：标准长代码（`510050C2609M02850`，主键）与新浪短代码（`10011255`，日线接口需要）。映射经逐合约 Greeks 接口懒建立、parquet 缓存；映射缺失显式返回 `NO_DATA`，绝不猜测。
